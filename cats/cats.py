...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    chosen = [i for i in paragraphs if select(i)]
    return chosen[k] if k < len(chosen) else ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def select(paragraph):
            for match in split(lower(remove_punctuation(paragraph))):
                if match in topic:
                    return True
            return False
    return select
...
def accuracy(typed, reference):
    ...
    for x, y in zip(typed_words, reference_words):
        if x == y:
            percentage += 1
    return percentage / len(typed_words) * 100.0
    # END PROBLEM 3
...
def sphinx_swaps(start, goal, limit):
    ...
    def helper(s, g, count):
        ...
        if len(s) == 0 or len(g) == 0:
            return helper(s[1:], g[1:], count + 1) + abs(len(s) - len(g))
        if s[0] == g[0]:
            return helper(s[1:], g[1:], count)
        else:
            return helper(s[1:], g[1:], count + 1)
...
def fastest_words(match):
    ...
    for x in word_indices:
        ...
        for y in player_indices:
            if time(match, y, x) < min_time:
                min_time = time(match, y, x)
                player = y
        fast[player].append(word_at(match, x))

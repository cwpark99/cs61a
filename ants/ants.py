...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.entrance is not None:
            if len(place.bees) and count >= self.min_range and count <= self.max_range:
...
class WallAnt(Ant):
    """
    WallAnt can block incoming bees.
    """
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class HungryAnt(Ant):
    """
    HungryAnt chews on a random Bee from its place and deals damage, but must chew for 3 turns after.
    """
    ...
    def action(self, gamestate):
        ...
        if self.chew_timer != 0:
            ...
            self.eat(random_bee(place.bees))
    ...
    def action(self, gamestate):
        ...
        while place.exit is not None:
            if place.ant is not None:
                ...
                if place.ant.is_container and place.ant.ant_contained is not None and not place.ant.ant_contained.is_doubled:
